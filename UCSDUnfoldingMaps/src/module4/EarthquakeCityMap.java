...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		...
		if (offline) {
			...
			map = new UnfoldingMap(this, 200, 50, 650, 600, new EsriProvider.WorldTerrain());
...
fill(255, 250, 240);
	...
	private void printQuakes() 
	{
		...
		HashMap<String, Integer> countryEqHash = new HashMap<String, Integer>();
		int oceanCount = 0;
		
		for (Marker eq : quakeMarkers) {
			if (eq instanceof LandQuakeMarker) {
				String country = (String)eq.getProperty("country");
				if (!countryEqHash.containsKey(country)) {
					countryEqHash.put(country, 0);
				}
				countryEqHash.put(country, countryEqHash.get(country)+1);
			} else {
				oceanCount += 1;
			}
		}
		...
		for (String c : countryEqHash.keySet()) {
			System.out.println("Number of earthquakes in " + c + ": " + countryEqHash.get(c));
		}
		System.out.println("Number of earthquakes in ocean: " + oceanCount);
