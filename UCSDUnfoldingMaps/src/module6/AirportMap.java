	...
	private void checkAirportsForClick() {
		...
		for (Marker m : airportList) {
			...
			if (!marker.isHidden() && marker.isInside(map, mouseX, mouseY)) {
				lastClicked = marker;
				List<Location> visableAirports = new ArrayList<Location>();
				
				for (Marker route : routeList) {
					SimpleLinesMarker slm = (SimpleLinesMarker) route;
					List<Location> airports = slm.getLocations();
					if (! airports.contains(lastClicked.getLocation())) {
						route.setHidden(true);
					} else {
						for (Location airport : airports) {
							if (! visableAirports.contains(airport)) {
								visableAirports.add(airport);
							}
						}
					}
				}
				...
				for (Marker mhide : airportList) {
					if (mhide != lastClicked && !visableAirports.contains(mhide.getLocation())) {
